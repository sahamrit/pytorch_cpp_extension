// Copied frpm https://github.com/pytorch/extension-cpp/blob/master/cuda/lltm_cuda_kernel.cu
...
namespace {
// __device__ suggests that it can be only called from the device 
// not the host. This is useful for utilities function which
// will be used only from device
// __inline__ functions avoid extra call
...
// We work with restricted types inside cuda kernels for efficiency
// Hence we don't have torch:: support here. All we have is other
// cuda and math libraries.
// cudNN, cuBLAS , cuFFT are some examples.
template <typename scalar_t>
__global__ void lltm_cuda_forward_kernel(
  ...
  // 2 : xxxx yyyy
  // 3 : xxxx yyyy
  // x & y are processed by block in x dim
  // batches are processed by block in y dim
  if (c < gates.size(2)){
...
std::vector<torch::Tensor> lltm_cuda_forward(
  ...
  // addmm already has highly optimised kernels implemented
  // Hence we are not reimplementing it
  ...
  // Note the AT_DISPATCH_FLOATING_TYPES internally has a switch
  // case which usign RTTI checks the type of gates
  // Based on it the dispatch to appropriate method is done
  // Since it supports only float and double, the corresponding
  // methods are already compiled.
